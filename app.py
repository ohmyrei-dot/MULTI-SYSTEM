...
# -----------------------------------------------------------------------------
# 4. [신규] 업체별 매입단가 조회 (쓰레기통 버튼 + 업체 열 순서 유지 + 초기값 없음)
# -----------------------------------------------------------------------------
def run_vendor_purchase_system():
    ...
    try:
        ...
        if add_item:
            ...
            with c_add2:
                add_spec_label = st.selectbox("규격 (규격1 | 규격2)", spec_opts, index=None, placeholder="규격을 선택하세요...", key="vp_new_spec")
