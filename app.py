...
def run_sales_system():
    ...
    try:
        ...
        # 컬럼명 유효성 검사 (사용자 요청 컬럼: 품목, 규격, 비고 1, 매출업체, 현재매출단가(포함))
        ...
        # 피벗 생성 (인덱스: 품목, 규격, 비고 / 컬럼: 매출업체 / 값: 동적으로 찾은 현재매출단가)
        ...
        if sel_vendor and sel_item_display:
            ...
            for col in df_sales.columns:
                # 메타 데이터 및 현재단가 컬럼 제외
                if col in ['품목', '규격', note_col, '매출업체', current_price_col, 'rank_item', 'rank_note', 'rank_spec']:
                    continue
