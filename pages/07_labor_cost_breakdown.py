...
try:
    ...
    if '규격1' in df_purch.columns: 
        df_purch['규격'] = df_purch['규격1']
    elif '규격' not in df_purch.columns: 
        df_purch['규격'] = ""
        
    if '규격2' in df_purch.columns: 
        note_col = '규격2'
    elif '비고 1' in df_purch.columns: 
        note_col = '비고 1'
    else: 
        note_col = '비고'
    ...
    df_purch['규격'] = df_purch['규격'].fillna("").astype(str)
    ...
    if not net_base_row.empty and pd.notna(net_base_row.iloc[0][price_col]):
        base_raw_price = float(net_base_row.iloc[0][price_col])
        # 만약 가격이 5000보다 크면 1롤(50m) 단위 가격으로 간주하여 해배(m²)당으로 변환
        if base_raw_price > 5000:
            spec_str = str(net_base_row.iloc[0]['규격']).replace(" ", "").lower()
    ...
    def extract_details(row):
        spec1_str = str(row['규격']).replace(" ", "").lower()
        ...
        # 폭 파악 (1m*50, 1.5*50 등)
        width = None
        w_match = re.search(r'(\d+(?:\.\d+)?)[a-z]*[*x]\s*50', spec1_str)
    ...
    if df_target.empty:
        ...
        st.info(f"💡 엑셀의 '규격1'에 폭(예: 1m*50), '{note_col}'에 두께(예: 8mm가공)가 있는지 확인해주세요.")
    else:
        results = []
        for _, row in df_target.iterrows():
            raw_price = row[price_col]
            if pd.isna(raw_price) or raw_price == 0: continue
