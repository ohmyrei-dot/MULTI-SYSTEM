...
for i, row in valid_rows.iterrows():
    ...
    tbody_html += f"""
    <tr>
        <td style='text-align:center; padding:2px 4px; border:1px solid #000;'>{r_no}</td>
        <td style='padding:2px 4px; border:1px solid #000;'>{r_item}</td>
        <td style='padding:2px 4px; border:1px solid #000;'>{r_spec}</td>
        <td style='text-align:center; padding:2px 4px; border:1px solid #000;'>{r_unit}</td>
        <td style='text-align:center; padding:2px 4px; border:1px solid #000;'>{r_qty}</td>
        <td style='text-align:right; padding:2px 4px; border:1px solid #000;'>{r_price}</td>
        <td style='text-align:right; padding:2px 4px; border:1px solid #000;'>{r_amt}</td>
        <td style='padding:2px 4px; border:1px solid #000;'>{r_note}</td>
    </tr>
    """
...
<div id="invoice-box" style="font-family: 'Malgun Gothic', sans-serif; max-width: 800px; margin: 0 auto; padding: 10px; border: 2px solid #333; background: #fff; color: #000;">
    ...
    <div style="display: flex; justify-content: space-between; margin-top: 15px; font-size: 13px;">
        <div style="width: 45%;">
            <table style="width: 100%; border-collapse: collapse;">
                <tr><td style="padding: 2px 0; border-bottom: 1px solid #000; font-weight: bold; width: 60px;">수신처</td>
                    <td style="padding: 2px 0; border-bottom: 1px solid #000;">{f"{q_recipient} 귀하" if q_recipient else ""}</td></tr>
                <tr><td style="padding: 2px 0; border-bottom: 1px solid #000; font-weight: bold;">참조</td>
                    <td style="padding: 2px 0; border-bottom: 1px solid #000;">{q_ref}</td></tr>
                <tr><td style="padding: 2px 0; border-bottom: 1px solid #000; font-weight: bold;">연락처</td>
                    <td style="padding: 2px 0; border-bottom: 1px solid #000;">{q_phone}</td></tr>
            </table>
            <div style="margin-top: 10px;">
                <p style="margin: 2px 0;"><strong>견적일 : </strong> {q_date.strftime('%Y년 %m월 %d일')}</p>
                <p style="margin: 2px 0;"><strong>견적명 : </strong> {q_name}</p>
                <p style="margin: 2px 0; font-size: 15px;"><strong>합계금액 : </strong> <span style="font-size: 16px; font-weight: bold;">₩ {int(total_sum):,}</span> (VAT 별도)</p>
            </div>
        </div>
        ...
        <div style="width: 45%;">
            <table style="width: 100%; border-collapse: collapse; border: 2px solid #000;">
                <tr>
                    ...
                    <td style="width: 90px; padding: 2px; border-right: 1px solid #000; border-bottom: 1px solid #000;">사업자번호</td>
                    <td style="padding: 2px; border-bottom: 1px solid #000;">{s_biznum}</td>
                </tr>
                <tr>
                    <td style="padding: 2px; border-right: 1px solid #000; border-bottom: 1px solid #000;">상호</td>
                    <td style="padding: 2px; border-bottom: 1px solid #000;"><strong>{s_company}</strong></td>
                </tr>
                <tr>
                    <td style="padding: 2px; border-right: 1px solid #000; border-bottom: 1px solid #000;">주소</td>
                    <td style="padding: 2px; border-bottom: 1px solid #000; font-size: 11px;">{addr_html}</td>
                </tr>
                <tr>
                    <td style="padding: 2px; border-right: 1px solid #000;">연락처</td>
                    <td style="padding: 2px; font-size: 11px;">{s_contact}<br>E-mail: {s_email}</td>
                </tr>
            </table>
        </div>
    </div>
        ...
        <table style="width: 100%; border-collapse: collapse; border: 2px solid #000; font-size: 12px;">
            <thead>
                <tr style="background-color: #f0f0f0;">
                    <th style="padding: 3px; border: 1px solid #000; width: 40px;">번호</th>
                    <th style="padding: 3px; border: 1px solid #000; width: 150px;">품명</th>
                    <th style="padding: 3px; border: 1px solid #000;">규격</th>
                    <th style="padding: 3px; border: 1px solid #000; width: 50px;">단위</th>
                    <th style="padding: 3px; border: 1px solid #000; width: 50px;">수량</th>
                    <th style="padding: 3px; border: 1px solid #000; width: 80px;">단가(원)</th>
                    <th style="padding: 3px; border: 1px solid #000; width: 90px;">금액(원)</th>
                    <th style="padding: 3px; border: 1px solid #000; width: 70px;">비고</th>
                </tr>
            </thead>
            <tbody>
                {tbody_html}
